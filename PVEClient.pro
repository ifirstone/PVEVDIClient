...
SOURCES += \
    ...
    src/core/XfreerdpDetector.cpp \
    src/core/EasyTierManager.cpp \
...
HEADERS += \
    ...
    src/core/EasyTierManager.h \
    src/core/DebugLogger.h \
    src/core/XfreerdpDetector.h \
...
# ========== 复制 resources/package 到 bin 目录 ==========
# 这确保 easytier-core 和 easytier-cli 一起打包到发布目录中
win32 {
    # 使用通配符确保只复制内容，不复制package目录本身
    QMAKE_POST_LINK += $$quote(cmd /c xcopy $$shell_path($$PWD/resources/package\\*) $$shell_path($$DESTDIR/package\\) /E /I /Y)
}

unix {
    # mkdir确保目标目录存在，/* 确保只复制内容不复制package目录本身
    QMAKE_POST_LINK += mkdir -p $$DESTDIR/package && cp -r $$PWD/resources/package/* $$DESTDIR/package/ 2>/dev/null; true
}
