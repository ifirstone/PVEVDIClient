...
{
    // 应用退出时确保 EasyTier 子进程被停止，避免残留后台进程
    if (m_connectionManager) {
        m_connectionManager->stopEasyTierRuntime();
    }
}
...
void MainWindow::setupUI()
{
    ...
    // 页面 0: 全屏登录视图
    m_loginView = new LoginView(m_configManager, m_authManager, m_connectionManager->easyTierManager(), this);
...
{
    SettingsDialog dialog(m_configManager, m_connectionManager->easyTierManager(), this);
...
{
    // 将主动关闭行为落盘给外部守护进程识别（例如 Alt+F4）。
    QString stateDir = qEnvironmentVariable("PVECLIENT_STATE_DIR");
    if (stateDir.isEmpty()) {
        stateDir = QDir::tempPath() + "/pveclient-state";
    }
