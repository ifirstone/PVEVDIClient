...
{
    // 第一步：根据用户配置的版本选择 xfreerdp
    // 版本号: 0=Auto, 2=v2 only, 3=v3 only
    QString program = getXfreerdpPath(info.rdpVersion);

    if (program.isEmpty()) {
        // 生成详细的错误信息
        QString errorMsg;
        if (info.rdpVersion == 2) {
            errorMsg = "未找到 FreeRDP 2.x 版本 (/usr/bin/xfreerdp)";
        } else if (info.rdpVersion == 3) {
            errorMsg = "未找到 FreeRDP 3.x 版本 (/usr/bin/xfreerdp3)";
        } else {
            errorMsg = "未找到任何 FreeRDP 版本，请确认系统已安装 xfreerdp 或 xfreerdp3 包";
        }
        qWarning() << errorMsg;
    ...
    // 第二步：构建参数列表
    QStringList args = buildArguments(info, program);

    // 第三步：如果启用了 USB 驱动器重定向，在启动前先同步一次，然后启动定时轮询
...
QStringList RdpLauncher::buildArguments(const ConnectionInfo &info, const QString &actualProgram) const
{
    QStringList args;
    ...
    // 基于实际找到并使用的二进制路径判断版本，而非用户的版本偏好配置
    // 这样才能确保参数语法与真实的 xfreerdp 二进制版本匹配
    bool isFreeRDP3 = actualProgram.contains("xfreerdp3");
...
{
    // 使用 auto 模式获取默认版本的 xfreerdp
    QString program = getXfreerdpPath(0);  // 0 = auto mode
...
QString RdpLauncher::getXfreerdpPath(int version) const
{
    return XfreerdpDetector::instance().getXfreerdpPath(version);
}
