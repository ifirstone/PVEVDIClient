...
private:
    // 构建 xfreerdp 参数列表
    QStringList buildArguments(const ConnectionInfo &info, const QString &actualProgram) const;

    // 根据配置版本获取 xfreerdp 可执行文件路径（严格模式）
    // 返回空字符串表示找不到该版本
    QString getXfreerdpPath(int version) const;
