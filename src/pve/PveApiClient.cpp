...
void PveApiClient::checkPermissions(const QString &path)
{
    // 查询当前用户在指定路径的权限
    // API 端点：GET /api2/json/access/permissions?path=/path/to/resource
    QString queryPath = "/api2/json/access/permissions";
    
    QUrl url(buildUrl(queryPath));
    QUrlQuery query;
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, path]() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            qWarning() << "权限检查失败:" << reply->errorString();
            emit permissionsReceived(path, QStringList());
            return;
        }
        ...
        // PVE 返回的权限是一个对象，键是权限字符串，值表示是否拥有
        ...
        qDebug() << "用户" << m_username << "在路径" << path << "的权限:" << permissions;
        emit permissionsReceived(path, permissions);
    });
}
// ========== 认证 ==========
